...
if ($row_check['current_users'] >= 2 && $_SESSION['in']!=1) {
    echo "<script>alert('すでに最大人数が参加しています。');
    window.location.href = 'chat_list_jp.php';
    </script>";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user_guess == $answer) {
        ...
        echo "<script>alert('正解です！ {$_SESSION['tries']}回で正解しました！');
        window.location.href = 'chat_list_jp.php';
        exit;</script>";
        ...
        $result = mysqli_query($db, $query);
        echo "<script>alert('3回の機会を使い切りました。');
        window.location.href = 'chat_list_jp.php';
        </script>";
        exit;

    } else {
        $mess = "{$_SESSION['tries']}回試しました。もう一度試してください。";
    }
}
?>
...
<header class="purple-theme-shadow">
      ...
      <div class="row">
        ...
        <div class="col-sm-4 offset-md-1 py-4" style="display: flex; justify-content: flex-end;">
            ...
            <div style="display: block; position: relative;">
                <?php if ($creator_id == $user_id) : ?>
                    <div class="chatroom-action">
                        <form action="delete_chatroom_jp.php" method="post">
                        <input type="hidden" name="chatroom_id" value="<?php echo $chatroom_id; ?>">
                        <button type="submit" class="delete-btn">チャットルーム削除</button>
                        </form>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div style="display: block; position: absolute; bottom: 6px;">
                <a href="chat_list_jp.php" class="text-white">チャットルームを出る</a>
            </div>
        </div>
      </div>
    </div>
  ...
  <div class="navbar navbar-dark purple-theme shadow-sm">
    <div class="container">
      <a href="chat_list_jp.php" class="navbar-brand align-items-center" >
        <i class="fa-regular fa-comments"></i>
        <strong style="margin-left: 5px;"><?php echo "$room_name / ルームオーナー : $creator_id"; ?></strong>
      </a>
        ...
        if($creator_id == $user_id) {
            echo 
                "<div style='text-align:center; margin-top: 20px;'>
                    <p><i class='fas fa-users'></i> 現在の参加者 : <span id='now_join'></span></p>
                </div>";
        }
    ?>
    ...
    <?php
        if($creator_id != $user_id) {
            if($ask_count >= 20) {
                ...
                <p id='ask_count_p'>機会をすべて使いました。</p>
                </div>";
            }
                
            else {
                echo "<div style='text-align: center'>
                <p id='ask_count_p'>これまでの質問回数: <span id='ask_count'>{$ask_count}</span></p>
                </div>";
